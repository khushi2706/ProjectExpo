...
export default function ProjectView(props) {
  ...
  const sendReqStudentDetails = async () => {
    console.log(user);
    const res = await axios
      .get(`http://localhost:5000/api/student/getStudentById/${user.UserId}`)
      .catch((err) => console.log(err));
    const data = await res.data;
    console.log(data);
    return data;
  ...
  const sendReqLike = async () => {
    const res = await axios
      .post(`http://localhost:5000/api/Project/addNewRate`, {
        projectId: id,
        RateTobeAdded: 1,
      })
      .catch((err) => console.log(err));
  ...
  const sendReqToDown = async () => {
    const res = await axios
      .get(`http://localhost:5000/api/Project/download/${id}`)
      .catch((err) => console.log(err));
    const data = await res.data;
  ...
  useEffect(() => {
    sendRequest().then((data) => {
      ...
      console.log(user);
      sendReqStudentDetails().then((data) => {
        setStudent(data);
      });
    });
            ...
            <div className="leftt">
              ...
              >
                ...
                <span style={{ fontSize: 16 }}>
                  Updated at {user.Date.substring(0, 10)}
                </span>
              </div>
              <div
                className=""
                style={{ color: "gray", textAlign: "left", marginLeft: "12px" }}
              >
                Prof :- Mohsin Hasan
              </div>
            </div>
